...
#   endif
	...
	if (area->resources[searchingResource1] > 0 || area->resources[searchingResource2] > 0) {
		lowestDestinationAreaReached = area;
		return dijkstraTowardsResourceOrArea(false);
	}

	while (area->resources[searchingResource1] == 0 && area->resources[searchingResource2] == 0) {
...
int Unit::adjustPathTowardsResource() {
	...
	if (reachedArea->resources[searchingResource1] > 0 || reachedArea->resources[searchingResource2] > 0) {
	...
	while (true) {
		...
		if (reachedArea->superArea->resources[searchingResource1] > 0 || reachedArea->superArea->resources[searchingResource2] > 0) {
		...
		if (reachedArea->lvl == lowestDestinationAreaReached->lvl - 1) {
			...
			if (lowestDestinationAreaReached->resources[searchingResource1] > 0 || lowestDestinationAreaReached->resources[searchingResource2] > 0) {
				return resetPathTowardsResource(reachedArea->lvl);
			}
			else {
...
#				endif
		...
		if (currentArea->superArea == destArea || currentArea->resources[searchingResource1] > 0 || currentArea->resources[searchingResource2] > 0) {
...
#				endif
		...
		while (current != NULL) {
			if (current->objectType == objectResource) {
				...
				if (resource->resourceType == searchingResource1 || resource->resourceType == searchingResource2) {
					tilesToBeTraveled = distance;
					return direction;
				}
			}
